...
$t = ($lang === 'bm') ? [
    'eyebrow'   => 'KATEGORI',
    'title'     => 'Kontraktor',
    'sub'       => 'Rekod kenderaan kontraktor di UiTM.',
    ...
    'empty_sub'  => 'Belum ada kenderaan kontraktor didaftarkan.',
    'delete_confirm' => 'Padam rekod ini?',
] : [
    'eyebrow'   => 'CATEGORY',
    'title'     => 'Contractor',
    'sub'       => 'Vehicle records for UiTM contractors.',
    ...
    'empty_sub'  => 'No contractor vehicles have been registered yet.',
    'delete_confirm' => 'Delete this record?',
];

$rows = [];
$result = mysqli_query($con, "SELECT * FROM `owner` WHERE status = 'Kontraktor' ORDER BY id DESC");
if ($result) {
...
<main class="page">
    <div class="page-head">
        ...
        <div class="actions">
            <a class="btn btn-primary" href="/vehicles/contractor/add.php"><i data-lucide="plus"></i> <?php echo htmlspecialchars($t['add']); ?></a>
        </div>
    </div>
    ...
    <div class="card flat mt-6">
        <?php if (count($rows) === 0): ?>
            <div class="text-center" style="padding:48px 24px;">
                ...
                <a class="btn btn-primary mt-4" href="/vehicles/contractor/add.php"><i data-lucide="plus"></i> <?php echo htmlspecialchars($t['add']); ?></a>
            </div>
        <?php else: ?>
        <form id="bulkDeleteForm" method="POST" action="/api/bulk_delete_api.php">
            <input type="hidden" name="vehicle_type" value="contractor">
        <table class="table" id="vehicleTable">
            ...
            ?>
                <tr>
                    ...
                    <td class="text-right">
                        <a class="btn btn-quiet" href="/vehicles/contractor/update.php?id=<?php echo $id; ?>" title="Edit"><i data-lucide="pencil"></i></a>
                    </td>
                </tr>
            <?php endforeach; ?>
            </tbody>
        </table>
        <?php echo bulk_delete_button(['endpoint' => '/api/bulk_delete_api.php']); ?>
        </form>
        <?php endif; ?>
    </div>
