...
$t = ($lang === 'bm') ? [
    'eyebrow' => 'Pesara', 'page_title' => 'Kemaskini kenderaan', 'sub' => 'Kemaskini maklumat kenderaan pesara.',
    'status' => 'Status', 'name' => 'Nama', 'phone' => 'No. Telefon', 'ic_number' => 'No. KP',
    'vehicle_type' => 'Jenis kenderaan', 'plate_number' => 'No. Kenderaan', 'model' => 'Model kenderaan', 'model_ph' => 'cth. PERODUA MYVI',
    'date' => 'Tarikh ambil pelekat', 'note' => 'Catatan', 'serial' => 'No. Siri Pelekat',
    'save' => 'Simpan', 'cancel' => 'Batal', 'update_success' => 'Kenderaan pesara berjaya dikemaskini.',
    'update_failed' => 'Gagal mengemaskini', 'name_placeholder' => 'Nama', 'phone_placeholder' => 'cth. 012-3456789',
    'ic_placeholder' => 'cth. 600101-01-1234', 'plate_placeholder' => 'cth. JSX 1234',
] : [
    'eyebrow' => 'Alumni', 'page_title' => 'Update vehicle', 'sub' => 'Update alumni vehicle details.',
    ...
    'date' => 'Sticker issue date', 'note' => 'Note', 'serial' => 'Sticker serial no.',
    'save' => 'Save', 'cancel' => 'Cancel', 'update_success' => 'Alumni vehicle updated.',
    'update_failed' => 'Failed to update', 'name_placeholder' => 'Name', 'phone_placeholder' => 'e.g. 012-3456789',
    'ic_placeholder' => 'e.g. 600101-01-1234', 'plate_placeholder' => 'e.g. JSX 1234',
];
...
if (isset($_POST['submit'])) {
    ...
    if (nv_schema_col_exists($con, 'owner', 'note')) { $set .= ", `note`='" . mysqli_real_escape_string($con, strtoupper(trim($_POST['note'] ?? ''))) . "'"; }
    if (mysqli_query($con, "UPDATE `owner` SET $set WHERE id=$id")) {
        echo "<script>alert('" . addslashes($t['update_success']) . "'); window.location.href='/vehicles/alumni/list.php';</script>"; exit;
...
<main class="page">
    ...
    <form class="card nv-stack gap-6" method="POST">
        <div class="nv-grid cols-2">
            <div class="field"><label class="field-label" for="status"><?= htmlspecialchars($t['status']) ?></label>
                <select class="select" id="status" name="status" required>
                    <?php foreach (['Pesara','Staf','Pelajar','Pelawat','Kontraktor'] as $opt): ?>
        ...
        <div class="nv-row end gap-2">
            <a class="btn btn-ghost" href="/vehicles/alumni/list.php"><i data-lucide="arrow-left"></i> <?= htmlspecialchars($t['cancel']) ?></a>
