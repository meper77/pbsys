...
// Get visitor vehicles data
$visitor_vehicles = [];
$no = 1;
$sql = "SELECT * FROM `owner` WHERE status = 'Pelawat'";
$result = mysqli_query($con, $sql);

if ($result) {
    while ($row = mysqli_fetch_assoc($result)) {
        $visitor_vehicles[] = $row;
    }
}
?>
    ...
    <div class="container">
        <div class="logo-container">
            ...
            <div class="system-title">
                <h1><i class="fas fa-user-clock"></i> <?php echo $t['page_title']; ?></h1>
...
<div class="main-content">
    <div class="content-container">
        <div class="content-title">
            <span><i class="fas fa-list-alt me-2"></i><?php echo $t['visitor_vehicles_list']; ?></span>
            <a href="/vehicles/visitor/add.php" class="add-btn">
                <i class="fas fa-plus-circle me-2"></i><?php echo $t['add_visitor_vehicle']; ?>
            </a>
        </div>
        ...
        <?php if (count($visitor_vehicles) > 0): ?>
        <div class="table-responsive">
            <table class="data-table table-striped" id="visitorTable">
                <thead>
                    <tr>
                        <th width="50"><?php echo $t['no']; ?></th>
                        <th width="120"><?php echo $t['visitor_id']; ?></th>
                        <th><?php echo $t['visitor_name']; ?></th>
                ...
                <tbody>
                    <?php 
                    $counter = 1;
                    foreach ($visitor_vehicles as $row): 
                        $id = $row['id'];
                        $visitorid = htmlspecialchars($row['idnumber']);
                        $visitorname = htmlspecialchars($row['name']);
                        $phone = htmlspecialchars($row['phone']);
                        $visitorplate = htmlspecialchars($row['platenum']);
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="action-buttons">
                                <a href="/vehicles/visitor/update.php?id=<?php echo $id; ?>" class="edit-btn">
                                    <i class="fas fa-edit me-1"></i><?php echo $t['edit']; ?>
                                </a>
                                <a href="/vehicles/visitor/delete.php?id=<?php echo $id; ?>" class="delete-btn" onclick="return confirm('<?php echo $lang == 'bm' ? 'Adakah anda pasti ingin memadam rekod ini?' : 'Are you sure you want to delete this record?'; ?>')">
        ...
        <div class="no-data">
            ...
            <h4><?php echo $lang == 'bm' ? 'Tiada Kenderaan Pelawat' : 'No Visitor Vehicles'; ?></h4>
            <p><?php echo $lang == 'bm' ? 'Tiada rekod kenderaan pelawat dijumpai.' : 'No visitor vehicle records found.'; ?></p>
            <a href="/vehicles/visitor/add.php" class="btn btn-primary mt-3">
                <i class="fas fa-plus-circle me-2"></i><?php echo $t['add_visitor_vehicle']; ?>
            </a>
        </div>
        <?php endif; ?>
...
$(document).ready(function() {
    ...
    $('#export-btn').on('click', function() {
        ...
        if (table.length) {
            ...
            var wb = XLSX.utils.table_to_book(tableClone[0], {sheet: "Visitor Vehicles"});
            XLSX.writeFile(wb, "visitor-vehicles-<?php echo date('Y-m-d'); ?>.xlsx");
        }
    });
});
</script>
