...
$t = ($lang === 'bm') ? [
    'eyebrow'   => 'KATEGORI',
    'title'     => 'Pelawat',
    'sub'       => 'Rekod kenderaan pelawat ke UiTM.',
    ...
    'empty_sub'  => 'Belum ada kenderaan pelawat didaftarkan.',
    'delete_confirm' => 'Padam rekod ini?',
] : [
    'eyebrow'   => 'CATEGORY',
    'title'     => 'Visitor',
    'sub'       => 'Vehicle records for UiTM visitors.',
    ...
    'empty_sub'  => 'No visitor vehicles have been registered yet.',
    'delete_confirm' => 'Delete this record?',
];

$rows = [];
$result = mysqli_query($con, "SELECT * FROM `owner` WHERE status = 'Pelawat' ORDER BY id DESC");
if ($result) {
...
<main class="page">
    <div class="page-head">
        ...
        <div class="actions">
            <a class="btn btn-primary" href="/vehicles/visitor/add.php"><i data-lucide="plus"></i> <?php echo htmlspecialchars($t['add']); ?></a>
        </div>
    </div>
    ...
    <div class="card flat mt-6">
        <?php if (count($rows) === 0): ?>
            <div class="text-center" style="padding:48px 24px;">
                ...
                <a class="btn btn-primary mt-4" href="/vehicles/visitor/add.php"><i data-lucide="plus"></i> <?php echo htmlspecialchars($t['add']); ?></a>
            </div>
        <?php else: ?>
        <table class="table" id="vehicleTable">
            ...
            ?>
                <tr>
                    ...
                    <td class="text-right">
                        <a class="btn btn-quiet" href="/vehicles/visitor/update.php?id=<?php echo $id; ?>" title="Edit"><i data-lucide="pencil"></i></a>
                        <a class="btn btn-quiet" href="/vehicles/visitor/delete.php?id=<?php echo $id; ?>" title="Delete" onclick="return confirm('<?php echo addslashes($t['delete_confirm']); ?>')"><i data-lucide="trash-2"></i></a>
                    </td>
                </tr>
            <?php endforeach; ?>
            </tbody>
