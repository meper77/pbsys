...
$text['bm'] = [
    ...
    'save' => 'Simpan',
    'cancel' => 'Batal',
    'visitor_name_placeholder' => 'Isi nama pelawat',
    'phone_placeholder' => 'Isi nombor telefon pelawat',
    'visitor_id_placeholder' => 'Isi nombor pas pelawat (4-5 digit)',
    ...
    'registration_success' => 'Kenderaan pelawat berjaya didaftar!',
...
// English
$text['en'] = [
    'page_title' => 'Add Visitor Vehicle',
    'system_name' => 'NEO V-TRACK - Vehicle Management & Monitoring System',
    'admin_role' => 'Administrator',
    'logout' => 'Log Out',
    'logout_confirm' => 'Are you sure you want to log out?',
    'add_visitor_vehicle' => 'Add Visitor Vehicle',
    'back_to_list' => 'Back to List',
    'visitor_name' => 'Visitor Name',
    'phone' => 'Phone Number',
    'visitor_id' => 'Visitor Pass Number',
    ...
    'save' => 'Save',
    'cancel' => 'Cancel',
    'visitor_name_placeholder' => 'Enter visitor name',
    'phone_placeholder' => 'Enter visitor phone number',
    'visitor_id_placeholder' => 'Enter visitor pass number (4-5 digits)',
    ...
    'registration_success' => 'Visitor vehicle registered successfully!',
...
if (isset($_POST['submit'])) {
    ...
    $check_platenum = mysqli_query($con, "SELECT * FROM owner WHERE platenum = '$platenum'");
    $error = array();
    ...
    <div class="container">
        ...
        <div class="collapse navbar-collapse" id="navbarNav">
            <ul class="navbar-nav ms-auto">
                ...
                <li class="nav-item">
                    <a class="nav-link active" href="/vehicles/visitor/list.php">
...
<div class="main-content">
    <div class="form-container">
        <div class="form-title">
            <h3><i class="fas fa-user-plus"></i> <?php echo $t['add_visitor_vehicle']; ?></h3>
            <a href="/vehicles/visitor/list.php" class="btn-cancel">
        ...
        <form method="POST" id="visitorVehicleForm">
            <input type="hidden" id="status" name="status" value="Pelawat">
            
            <div class="mb-4">
                <label for="name" class="form-label">
                    <i class="fas fa-user me-2"></i><?php echo $t['visitor_name']; ?>
                </label>
                <input type="text" id="name" name="name" 
                       placeholder="<?php echo $t['visitor_name_placeholder']; ?>" 
            ...
            <div class="mb-4">
                <label for="idnumber" class="form-label">
                    <i class="fas fa-id-card me-2"></i><?php echo $t['visitor_id']; ?>
                </label>
                <input type="text" id="idnumber" name="idnumber" 
                       placeholder="<?php echo $t['visitor_id_placeholder']; ?>" 
                       class="form-control" required maxlength="5">
                <small class="text-muted" style="font-size: 12px;">
                    <i class="fas fa-info-circle me-1"></i>
                    <?php echo $lang == 'bm' ? '4-5 digit nombor' : '4-5 digit number' ?>
                </small>
            </div>
...
$(document).ready(function() {
    ...
    // Visitor Pass validation (4-5 digits only)
    ...
    // Form validation
    $('#visitorVehicleForm').on('submit', function() {
        let valid = true;
        ...
        // Validate visitor pass number format (4-5 digits)
        var passNumber = $('#idnumber').val().replace(/\D/g, '');
        if (passNumber) {
            if (passNumber.length < 4 || passNumber.length > 5) {
                $('#idnumber').addClass('is-invalid');
                alert('<?php echo $lang == "bm" ? "Nombor pas pelawat mesti mengandungi 4-5 digit." : "Visitor pass number must contain 4-5 digits."; ?>');
                valid = false;
            }
            
            // Check if it's all digits (already enforced by input handler)
            if (!/^\d+$/.test(passNumber)) {
                $('#idnumber').addClass('is-invalid');
                alert('<?php echo $lang == "bm" ? "Nombor pas pelawat hanya boleh mengandungi nombor." : "Visitor pass number can only contain numbers."; ?>');
                valid = false;
            }
        }
        
        // Validate plate number format
        var plateNumber = $('#platenum').val().trim();
        if (plateNumber && !/^[A-Z0-9\s\-]+$/.test(plateNumber)) {
            $('#platenum').addClass('is-invalid');
            alert('<?php echo $lang == "bm" ? "Format nombor plat tidak sah. Gunakan huruf besar, nombor, dan sela sahaja." : "Invalid plate number format. Use uppercase letters, numbers, and spaces only."; ?>');
            valid = false;
        }
