...
if (isset($_GET['id'])) {
    ...
    if ($admin_count <= 1) {
        echo "<script>alert('Tidak boleh membuang admin terakhir!'); window.location.href='admin.php';</script>";
        exit();
    }
    ...
    if ($current_admin_query && mysqli_num_rows($current_admin_query) > 0) {
        ...
        if ($id == $current_admin_id) {
            echo "<script>alert('Anda tidak boleh membuang akaun sendiri!'); window.location.href='admin.php';</script>";
            exit();
        }
    }
    ...
    if ($result) {
        if (mysqli_affected_rows($con) > 0) {
            echo "<script>alert('Admin berjaya dibuang!'); window.location.href='admin.php';</script>";
            exit();
        } else {
            echo "<script>alert('Admin tidak ditemui!'); window.location.href='admin.php';</script>";
            exit();
        }
    } else {
        ...
        if ($deleted) {
            ...
            echo "<script>alert('$error_msg'); window.location.href='admin.php';</script>";
            exit();
        }
    }
} else {
    echo "<script>alert('ID Admin tidak sah!'); window.location.href='admin.php';</script>";
    exit();
}
?>